...
func (sw *streamWatcher) receive() {
	...
	for {
		obj, err := sw.Decode()
		log.Printf("obj, err %v. %v", obj, err)
		if err != nil {
			...
			case io.ErrUnexpectedEOF:
				log.Printf("Unexpected EOF during watch stream event decoding: %v", err)
			default:
				log.Printf("Unable to decode an event from the watch stream: %v", err)
			}
			return
		}
		sw.result <- obj
